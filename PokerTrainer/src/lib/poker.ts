...
export const isCallCorrect = (equity: number, potOdds: number): boolean => {
  // Use >= so that if equity is exactly equal to pot odds, calling is correct
...
// Generate a random pot size appropriate for the street and player count
export const getRandomPotSize = (
  street: Street,
  playerCount: number = 2,
  bigBlind: number = 2
): number => {
  // Pot size increases with more players
  const playerMultiplier = 1 + 0.5 * (playerCount - 2); // 2 players = 1x, 3 = 1.5x, 4 = 2x, etc.
  switch (street) {
    case "preflop":
      return Math.round(
        bigBlind * (Math.floor(Math.random() * 5 + 1) * playerMultiplier)
      );
    case "flop":
      return Math.round(
        bigBlind * (Math.floor(Math.random() * 10 + 5) * playerMultiplier)
      );
    case "turn":
      return Math.round(
        bigBlind * (Math.floor(Math.random() * 20 + 10) * playerMultiplier)
      );
    case "river":
      return Math.round(
        bigBlind * (Math.floor(Math.random() * 40 + 20) * playerMultiplier)
      );
  }
};
...
export const calculateSimplifiedEquity = (
  ...
  // Postflop equity approximation
    ...
    const hasTwoPair =
      Object.values(rankFrequency).filter((freq) => freq >= 2).length >= 2;
    ...
    if (hasQuads) {
      ...
      equity = 0.75;
      equity = Math.max(0.25, equity - 0.02 * (playerCount - 2));
    } else if (hasTwoPair) {
      equity = 0.65;
      equity = Math.max(0.2, equity - 0.02 * (playerCount - 2));
    } else if (hasPair) {
      // Check if top pair
      const boardHighCard = Math.max(
        ...communityCards.map((c) => rankValues[c.rank])
      );
      const isPairWithHighCard = Object.entries(rankFrequency).some(
        ([rank, freq]) => freq >= 2 && Number(rank) === boardHighCard
      );

      equity = isPairWithHighCard ? 0.6 : 0.5;
      equity = Math.max(0.1, equity - 0.03 * (playerCount - 2));
    } else if (hasFlushDraw) {
      equity = 0.4;
      equity = Math.max(0.08, equity - 0.03 * (playerCount - 2));
    } else if (hasOpenEndedStraightDraw) {
      equity = 0.35;
      equity = Math.max(0.07, equity - 0.03 * (playerCount - 2));
    } else {
      // High card value
      const highestCardInHand = Math.max(
        ...playerHand.map((c) => rankValues[c.rank])
      );
      if (highestCardInHand >= 13) {
        // K or A
        equity = 0.3;
      } else if (highestCardInHand >= 10) {
        // T, J, Q
        equity = 0.25;
      } else {
        equity = 0.2;
      }
      equity = Math.max(0.05, equity - 0.03 * (playerCount - 2));
    }

    // Apply street penalties as before (except for quads/full house/flush)
    if (!(hasQuads || (hasTrips && hasPair) || hasFlush)) {
      if (street === "flop") {
        equity = equity * 0.9;
      } else if (street === "turn") {
        equity = equity * 0.95;
...
export const generateExplanation = (
  ...
  let explanation = `You have ${formatCard(playerHand[0])}${formatCard(
    playerHand[1]
  )}`;

  if (communityCards.length > 0) {
    explanation += ` with a board of ${communityCards
      .map(formatCard)
      .join("")}`;
  }

  explanation += `.\n\nThe pot is ${potSize} and you need to call ${betToCall}. `;
  explanation += `Your pot odds are ${(potOdds * 100).toFixed(
    1
  )}% (${betToCall}/${potSize + betToCall}). `;
  explanation += `Your equity in this spot is approximately ${(
    equity * 100
  ).toFixed(1)}%.\n\n`;

  if (isGoodCall) {
    explanation += `Since your equity (${(equity * 100).toFixed(
      1
    )}%) is greater than your pot odds (${(potOdds * 100).toFixed(
      1
    )}%), calling is the correct GTO play. `;
    explanation += `For every 100 units you invest, you'll win back more than 100 units in the long run.`;
  } else {
    explanation += `Since your equity (${(equity * 100).toFixed(
      1
    )}%) is less than your pot odds (${(potOdds * 100).toFixed(
      1
...
export const generateRandomGameState = (
  ...
  const potSize = getRandomPotSize(street, playerCount);
  const betToCall = getRandomBetSize(potSize);

  // Calculate equity
  const equity = calculateSimplifiedEquity(
    playerHand,
    communityCards,
    playerCount,
    street
...
// Edge case tests for isCallCorrect
if (typeof window !== "undefined") {
  // @ts-ignore
  if (window.__POKER_TRAINER_TESTS__ !== false) {
    const testCases = [
      { equity: 0.333, potOdds: 0.333, expected: true },
      { equity: 0.334, potOdds: 0.333, expected: true },
      { equity: 0.332, potOdds: 0.333, expected: false },
      { equity: 0.5, potOdds: 0.5, expected: true },
      { equity: 0.499, potOdds: 0.5, expected: false },
      { equity: 0.501, potOdds: 0.5, expected: true },
    ];
    testCases.forEach(({ equity, potOdds, expected }, i) => {
      const result = isCallCorrect(equity, potOdds);
      // eslint-disable-next-line no-console
      console.log(
        `Test ${
          i + 1
        }: equity=${equity}, potOdds=${potOdds} => ${result} (expected: ${expected})`,
